...
| `tools/build.py` | Rebuilds `index.html` from `src/` and the dataset. Only needed if you change the app. |
...
`index.html` is generated by `tools/build.py` from everything in `src/`. If you only want
to use the app, you can ignore `src/`, `tools/` and `tests/` entirely -- `index.html` is
complete on its own.

## Deploying

The site is served straight from these files -- there is no build step to run. Under
**Settings → Pages → Source**, choose **Deploy from a branch**, then `main` and `/ (root)`.
GitHub publishes `index.html` within a minute or so. Every later change you commit goes
live automatically.

The published site is readable by anyone.

The source PDFs are not included here. `tools/` documents how the dataset was derived, but
re-running it needs your own copy of the books.
